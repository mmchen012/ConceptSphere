...
# Compile-AhkV2.ps1
# Stop -> compile (AHK v2, 64-bit), for the .ahk passed in. The .exe is written
# next to the .ahk, replacing whatever is there.
#
# It does NOT start anything afterwards: a running copy is stopped only because
# Windows locks a running .exe and it could not otherwise be replaced. Start it
# again yourself when you want it.
#
...
try {
    ...
    # Purely to release the file lock: a running .exe cannot be overwritten.
    # Stopping through its scheduled task first is the clean way to end it.
    ...
    $stopNote = 'It was not running, so nothing was stopped.'
    if ($wasRunning) {
        $stopNote = if ($task) {
            "It was stopped (task $($task.TaskPath)$($task.TaskName)) and has NOT been restarted."
        } else {
            'It was stopped and has NOT been restarted.'
        }
        Write-Host 'Not restarted, by design.' -ForegroundColor Yellow
    }
