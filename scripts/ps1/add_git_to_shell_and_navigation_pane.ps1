...
# Pin the Git Shell folder to the File Explorer navigation pane.
New-ItemProperty `
    -Path $ClsidPath `
...
# Verify that the namespace is pinned before restarting Explorer.
$PinnedToNavigationPane = Get-ItemPropertyValue `
    -Path $ClsidPath `
    -Name "System.IsPinnedToNameSpaceTree"

if ([int]$PinnedToNavigationPane -ne 1) {
    throw "Git was registered, but navigation-pane pinning verification failed."
}
...
Write-Host ""
Write-Host "Git Shell namespace installed successfully."
Write-Host "Desktop namespace: available"
Write-Host "Navigation pane:   pinned"
