...
; By default AHK runs ONE thread per hotkey: while a handler is still running,
; further presses of that key are DISCARDED -- they reach neither the handler
; nor Start, so the key simply vanishes. That is the "Tab does nothing" wedge.
; Allow re-entry; the busy guards below turn the extra press into a native key.
A_MaxThreadsPerHotkey := 3
...
tabBusy := false        ; same, for TabJump (see the guard in TabJump)
tabBusyAt := 0
jumpFailAt := 0         ; when the direct jump to the app list last failed

; Start's own all-apps section sometimes fails to populate: AppsList ends up
; holding nothing but the pinned tiles, so there is no app row for anything to
; focus. Restarting StartMenuExperienceHost rebuilds it. AUTO_RESTART_START
; makes that happen the moment the fault is detected; set it false to be told
; about it (a low double tone) and restart by hand with Ctrl+Alt+Shift+T.
AUTO_RESTART_START := true
RESTART_COOLDOWN := 60000                ; ms; never restart Start more often
lastStartRestart := 0
...
menuAnchorFolder := ""  ; folder the row lives in, if the menu came from one
...
InContextMenu(el) {
    ...
    Loop 15 {
        ...
        if (t = CT_MENU || t = CT_MENUITEM)
            return true
        ; The Start context menu is NOT a UIA Menu -- it is Chromium HTML, so it
        ; surfaces as a LIST named "Context Menu" with ListItem children. Testing
        ; only for Menu/MenuItem missed it entirely, which is why the pin-restore
        ; never ran.
        nm := ""
        try nm := cur.Name
        if InStr(StrLower(nm), "context menu")
            return true
        p := 0
        try p := cur.Parent
        cur := p
...
TabJump(*) {
    global tabBusy, tabBusyAt
    ; Finding the app list can take a moment. If a previous Tab is still working,
    ; send a plain native Tab rather than letting this press be swallowed -- a
    ; lost key looks exactly like the script having crashed.
    if tabBusy {
        if (A_TickCount - tabBusyAt > 5000) {
            Log("tabBusy stuck -> cleared")
            tabBusy := false
        } else {
            Log("TabJump: busy -> passing Tab through")
            Send("{Tab}")
            return
        }
    }
    tabBusy := true
    tabBusyAt := A_TickCount
    try TabJumpCore()
    tabBusy := false
}

TabJumpCore() {
    global listMode, CT_EDIT, jumpFailAt
    focused := SafeFocus()
    ...
    if IsPinnedToggle(focused) || IsViewToggle(focused) {
        ; ...EXCEPT right after a failed jump. Otherwise this closes a loop:
        ; pinned -> (jump fails) -> native Tab -> toggle -> back to pinned -> ...
        ; which is exactly being trapped between the pinned grid and "Show all
        ; pinned apps", unable to reach the app list, account, power or search.
        if (A_TickCount - jumpFailAt < 3000) {
            Log("TabJump: at a toggle just after a failed jump -> native Tab")
            Send("{Tab}")
            return
        }
        if FocusFirstPinned()
    ...
    if FocusFirstApp()
        return
    jumpFailAt := A_TickCount
    TabPastToggles()
}

; The direct jump failed, so fall back to the native tab chain -- but do not stop
; ON a toggle. With several rows of pinned apps the next stop is "Show all/less
; pinned apps", and stopping there is where the pinned<->toggle loop begins.
; Step past toggle stops to whatever follows, which is normally the app list.
TabPastToggles() {
    Send("{Tab}")
    Loop 3 {
        Sleep(45)
        f := SafeFocus()
        if !(IsPinnedToggle(f) || IsViewToggle(f)) {
            Log("  TabPastToggles -> " DescribeFocus(f))
            return
        }
        nm := ""
        try nm := f.Name
        Log("  TabPastToggles: stepping past '" nm "'")
        Send("{Tab}")
    }
}
...
FocusFirstApp() {
    ...
    if IsObject(btn) {
        nm := ""
        try nm := btn.Name
        Log("  FocusFirstApp: overflow toggle is '" nm "'")
    ...
    ; Fingerprint of the Start-side fault: AppsList holds exactly the pinned
    ; tiles and nothing else. Healthy, it is far larger (pinned + every app row).
    if AppListIsEmpty() {
        Log("  FocusFirstApp: app list is EMPTY -- Start needs restarting")
        HandleEmptyAppList()
        return false
    }

    Log("  FocusFirstApp: GAVE UP -> native Tab")
    return false                          ; give up quietly; TabJump sends one Tab
}

; True when AppsList contains only the pinned tiles -- i.e. Start's all-apps
; section never populated. Compares counts rather than names: cheap, and the two
; lists are identical in that state.
AppListIsEmpty() {
    apps := GetAppsList()
    pinned := GetListById("PinnedList")
    if !(IsObject(apps) && IsObject(pinned))
        return false
    a := 0, p := 0
    try a := apps.FindElements({Type:"ListItem"}).Length
    try p := pinned.FindElements({Type:"ListItem"}).Length
    if (!a || !p)
        return false
    Log("  AppsList=" a " PinnedList=" p)
    return (a = p)
}

HandleEmptyAppList() {
    global AUTO_RESTART_START, RESTART_COOLDOWN, lastStartRestart
    if (!AUTO_RESTART_START || A_TickCount - lastStartRestart < RESTART_COOLDOWN) {
        PlayBeep()                        ; two low tones = "Start needs a restart"
        Sleep(120)
        PlayBeep()
        return
    }
    lastStartRestart := A_TickCount
    RestartStartMenu()
}

; Kill the Start menu host (and the search host). Windows respawns both within a
; second or so, with the all-apps list rebuilt. Start closes as a side effect --
; unavoidable, and in this state it was unusable anyway.
RestartStartMenu(*) {
    global lastStartRestart, HighBuf
    lastStartRestart := A_TickCount
    Log("restarting StartMenuExperienceHost")
    try RunWait('taskkill /f /im StartMenuExperienceHost.exe', , "Hide")
    try RunWait('taskkill /f /im SearchHost.exe', , "Hide")
    ProcessWait("StartMenuExperienceHost.exe", 5)
    Sleep(400)
    PlayTone(HighBuf)                     ; high tone = Start was restarted
}
...
TryFocusFirstApp() {
    list := GetAppsList()
    if !IsObject(list) {
        Log("  FocusFirstApp: AppsList not found")
        return false
    }
    ...
    if !IsObject(target) {
        cnt := 0, firstNm := ""
        try {
            items := list.FindElements({Type:"ListItem"})
            cnt := items.Length
            if cnt
                firstNm := items[1].Name
        }
        Log("  FocusFirstApp: no app row -- AppsList has " cnt " items, first='" firstNm "'")
        return false
    }
...
ContextMenuKey(key, *) {
    global menuAnchorName, menuAnchorInFolder, menuAnchorFolder
    focused := SafeFocus()
    menuAnchorName := ""
    menuAnchorFolder := ""
    ...
    if menuAnchorInFolder
        menuAnchorFolder := FolderModalName()   ; so we can get back if it closes
    Log("context menu opened on '" menuAnchorName "'"
        . (menuAnchorInFolder ? " (in folder '" menuAnchorFolder "')" : ""))
    Send(key)
}

; The expanded-folder modal element, or 0.
GetFolderModal() {
    hwnd := WinActive("ahk_exe StartMenuExperienceHost.exe")
    if !hwnd
        hwnd := WinExist("ahk_exe StartMenuExperienceHost.exe")
    ...
    try {
        root := UIA.ElementFromHandle(hwnd)
        return root.FindElement({AutomationId:"StartFolderModal"})
    }
    return 0
}

; The folder's display name. Start names the modal "<folder> expanded".
FolderModalName() {
    m := GetFolderModal()
    if !IsObject(m)
        return ""
    nm := ""
    try nm := m.Name
    return Trim(RegExReplace(nm, "i)\s*expanded\s*$"))
}

; First item in a list whose name starts with the given text.
FindByPrefix(list, prefix) {
    if (prefix = "")
        return 0
...
EnterKey(*) {
    ...
    Send("{Enter}")
    ; Gate on the ANCHOR, not on detecting a context menu: whether Enter landed
    ; in one is exactly the thing that was being got wrong, and an anchor is only
    ; ever set by opening a menu from a list row in the first place.
    if (menuAnchorName = "")
        return
...
RestoreMenuAnchor() {
    ...
    if menuAnchorInFolder {
        ...
        ; Pinning can close the folder. Its items are then not in the app list at
        ; all, so land on the folder's own row instead of giving up.
        if (menuAnchorFolder != "") {
            l2 := GetAppsList()
            if IsObject(l2) {
                row := FindByPrefix(l2, menuAnchorFolder)
                if IsObject(row) {
                    try row.ScrollIntoView()
                    try row.SetFocus()
                    Sleep(40)
                    Log("folder closed -> restored to folder row '" menuAnchorFolder "'")
                    return true
                }
            }
        }
        Log("restore FAILED: '" menuAnchorName "' not found in folder '" menuAnchorFolder "'")
        return false
    }
...
;   Ctrl+Alt+Shift+T  restart the Start menu (rebuilds an empty all-apps list)
#SuspendExempt
^!+s::ToggleSuspend()
^!+r::ReloadScript()
^!+t::RestartStartMenu()
