...
; =============================================================================
;  MiniTray.ahk  -  v2.3 (restore focus after title speech)
;
...
global RESTORE_FOCUS_SETTLE := 120
...
global POPUP_SETTLE := 120        ; ms to let the popup finish closing before a
                                  ;   restore runs -- see PopupThen()
global POPUP_OPEN_SETTLE := 140   ; let all dialog/list accessibility events enter
                                  ;   NVDA while the quiet gate is still closed
global POPUP_OPEN_TAIL := 300     ; keep late duplicate focus events behind the
                                  ;   single "MiniTray menu" announcement
...
; Pass Alt+Tab through to Windows, but hide the popup synchronously first.
; The tilde preserves the native task-switch operation; +ToolWindow below
; keeps MiniTray itself out of the Alt+Tab list.
    ...
    try {
        ...
        if (exe = "explorer.exe") {
            ...
            if (!g_ShellPid || pid = g_ShellPid) {
                SayQuietFinal("Windows Explorer not hidden. Enable separate Explorer processes.")
            ...
            if DllCall("User32\IsWindowVisible", "Ptr", hwnd) {
                ...
                SayQuietFinal("Windows Explorer could not be hidden safely")
                speechHandedOff := true
                return
            }
        } else {
...
; Called only after NVDA has finished speaking the explicit app/title line.
; The focus event itself is held behind a fresh short quiet window and then
; reported once by NVDA. For terminal controls, reportFocus includes the
; current caret line, which is the shell prompt.
QueueFocusAfterTitle(text, e) {
    ...
    if (!e || !e.HasOwnProp("focus") || !e.focus) {
        SayQuietFinal(text)
        return
    }
    ...
    if (g_Cfg.Get("Speak", "0") != "1") {
        EndQuiet()
        SetTimer(FocusRecordNow.Bind(e), -80)
        return
    }
    if (g_Cfg.Get("QuietPlugin", "1") != "1") {
        ...
        SetTimer(FocusRecordNow.Bind(e), -delay)
        return
    }
    ...
    ; Keep refreshing the quiet window until the callback arrives. If either
    ; process dies, the plugin's own per-sentinel cap still reopens speech.
...
CompleteDeferredFocus(token) {
    global g_DeferredFocus, RESTORE_FOCUS_SETTLE
    ...
    ; The user may have deliberately switched away while the title was being
    ; spoken. Never steal focus back in that case.
    if (!WinExist("ahk_id " rec.hwnd)
            || DllCall("User32\GetForegroundWindow", "Ptr") != rec.hwnd) {
        EndQuiet()
        return
    }

    StartQuiet(RESTORE_FOCUS_SETTLE + 500)
    FocusRecordNow(rec)
    SetTimer(ReportRestoredFocus, -RESTORE_FOCUS_SETTLE)
}
...
ReportRestoredFocus(*) {
    global g_QuietUntil
    ; MTREPORTFOCUS atomically reopens NVDA speech and reports its current focus.
    ; Do not send MTQUIET:0 first: that would create a gap in which the normal
    ; focus event could escape and duplicate the explicit report.
    g_QuietUntil := 0
    SetTimer(QuietRefresh, 0)
    Announce(Chr(1) "MTREPORTFOCUS" Chr(1), false)
}
...
RestoreOne(hwnd, announceWindow := false, *) {
    ...
    ; Every individual restore now uses the same deterministic order:
    ; app/title first, then the previously focused control.

    appName := AppNameFor(e.hwnd)
    if (appName = "")
        appName := e.proc

    StartQuiet(SPEECH_SETTLE + 400)
    ; Activate the top-level window now, but leave its internal control alone
    ; until the app/title line has finished speaking.
    Reveal(e, true, false)
    Forget(hwnd)
    SaveHidden(), RefreshTray()

    ; Individual restores always identify the window explicitly. After that
    ; utterance completes, the NVDA callback returns focus to e.focus and
    ; reports it once; terminal controls therefore speak their current prompt.
    said := appName " " e.title
    SayAfterTrayChange(said, e)
}
...
RestoreLast(*) {
    ...
    if (g_Hidden.Length = 1) {
        RestoreOne(g_Hidden[1].hwnd, true)
        return
    }
    ...
    if (appName = "")
        appName := e.proc
    SayAfterTrayChange(appName " " e.title, e)
}
...
;       Chr(1) "MTREPORTFOCUS" Chr(1)     report the now-restored focused item
;
...
FlushPendingSay() {
    ...
    if (text != "") {
        ...
        EndQuiet()
        Announce("")                    ; cancel the tray re-read, say nothing
        if focusEntry
            SetTimer(FocusRecordNow.Bind(focusEntry), -80)
    }
}
...
;   Up / Down     move within the current level and wrap at both ends
;   Left / Right  cross between the application and window levels; either
;                 direction wraps across the outer boundary
...
BeginTrayPopupOpen(*) {
    ...
    ; Start suppression before the GUI exists. Its dialog, list and
    ; unselected-item focus events are otherwise queued before
    ; ShowTrayPopup gets a chance to silence them.
    g_PopupOpening := true
    StartQuiet(POPUP_OPEN_SETTLE + POPUP_OPEN_TAIL + 500)
    SetTimer(ShowTrayPopup, -1)          ; get off the tray message handler
}

ShowTrayPopup(*) {
    global g_PopupGui, g_PopupLB, g_PopupProc, POPUP_OPEN_SETTLE
    if !g_PopupGui
        BuildTrayPopup()
    ...
    try g_PopupLB.Focus()
    SetTimer(AnnounceTrayPopupOpened, -POPUP_OPEN_SETTLE)
}
...
AnnounceTrayPopupOpened() {
    ...
    SayQuietFinal(APP_NAME " menu", POPUP_OPEN_TAIL)
}
...
HideTrayPopup(*) {
    global g_PopupGui, g_PopupProc, g_PopupOpening
...
FillTrayPopup(selectIndex := 1, selectProc := "") {
    ...
    if (g_PopupProc != "" && grp.groups.Has(g_PopupProc)) {
        ...
        ; No "back" row: either horizontal arrow returns to applications.
    ...
    ; Returning from a window menu reselects its application. Initial opening
    ; passes zero, so NVDA hears the list itself but no first item until Down.
    if (selectProc != "") {
        for idx, it in g_PopupItems {
            if (it.kind = "app" && it.proc = selectProc) {
                selectIndex := idx
                break
            }
        }
...
PopupActivate(*) {
    ...
    if (it.kind = "app") {
        ...
        ; Either horizontal arrow opens it to pick out one window.
...
; Horizontal navigation wraps across the two menu levels. Existing
; directions remain valid (Apps Right -> Windows, Windows Left -> Apps), and
; the opposite boundary directions now cycle too (Apps Left -> Windows,
; Windows Right -> Apps).
PopupDrillIn(*) {
    global g_PopupProc
    if (g_PopupProc != "") {
