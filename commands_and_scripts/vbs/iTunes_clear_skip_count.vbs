...
' ClearSkipCount
' ==============
' Version 1.0.0.6 - December 2nd 2011
' Copyright © Steve MacGuire 2011
' http://samsoft.org.uk/iTunes/ClearSkipCount.vbs
...
' Description
' ===========
' Reset skip counts/dates of selected tracks or current playlist

' Related scripts: ClearBookmarks, ClearSkipCount, LastPlayedNever

' =========
' ChangeLog 
' =========
' Version 1.0.0.1 - Initial version
' Version 1.0.0.2 - Updated to incorporate progress bar
' Version 1.0.0.3 - Updated to to display running time
' Version 1.0.0.4 - Updated to new common code base
' Version 1.0.0.5 - Updated timing routines
' Version 1.0.0.6 - Minor tweaks
...
Const Title="Clear Skip Count"
Const Summary="Reset skip counts."
...
' Reset skip counts/dates
' Modified 2011-10-23
Sub Action(T)
  With T
    StartEvent          ' Time potentially slow event
    'If .PlayedCount>0 Then .PlayedCount=0
    'If .PlayedDate>0 Then .PlayedDate=0
    If .SkippedCount>0 Then .SkippedCount=0
    If .SkippedDate>0 Then .SkippedDate=0
    'If .RememberBookmark And .BookmarkTime>0 Then .BookmarkTime=0
...
Function Prompt(T)
  ...
  With T
    A=.AlbumArtist : If A="" Then A=.Artist : If A="" Then A="Unknown Artist"
    B=.Album : If B="" Then B="Unknown Album"
    Prompt="Update?" & nl & nl & "Artist" & tab & tab & A & nl & "Album" & tab & tab & B & nl _ 
      & "Name" & tab & tab & .Name & nl & "Track #" & tab & tab &.TrackNumber & nl
    'If .PlayedCount>0 Or .PlayedDate>0 Then
    '  Prompt=Prompt & nl & "Play Count" & tab & .PlayedCount & nl & "Last Played" & tab & FormatDate(.PlayedDate)
    'End If
    If .SkippedCount>0 Or .SkippedDate>0 Then
      Prompt=Prompt & nl & "Skip Count" & tab & .SkippedCount & nl & "Last Skipped" & tab & FormatDate(.SkippedDate)
    End If
    'If .RememberBookmark And .BookmarkTime>0 Then
    '  Prompt=Prompt & nl & "Bookmark" & tab & tab & FormatTime(.BookmarkTime)
    'End If
  End With
End Function
...
Function Updateable(T)
  With T
    If .Location="" Then        ' Missing files can't be processed by this script
    ...
    Else                        ' No updating values which won't change
      Updateable=.SkippedCount>0 Or .SkippedDate>0
    End If
  End With
End Function
