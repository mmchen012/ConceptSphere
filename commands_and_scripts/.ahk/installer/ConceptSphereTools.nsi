...
!define MUI_ICON   "${PAYLOAD}\MiniTray.ico"
!define MUI_UNICON "${PAYLOAD}\MiniTray.ico"
...
Section "-Post" SEC_POST
  ...
  CreateShortcut "$SMPROGRAMS\ConceptSphere\Uninstall ${APPNAME}.lnk" "$INSTDIR\uninstall.exe"
  ...
  WriteRegStr HKLM "${REGKEY}" "DisplayIcon"     "$INSTDIR\MiniTray.exe"
